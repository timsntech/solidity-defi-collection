...
contract BeeNBeeMatic {
    ...
    function compoundHoney(bool isCompound) public {
        ...
        if (isCompound) {
            require(
                block.timestamp.sub(user.lastCompoundTimestamp) >=
                    COMPOUND_INTERVAL,
                "Compound interval not yet reached."
            );
        ...
        if (
            block.timestamp.sub(user.lastCompoundTimestamp) >= COMPOUND_INTERVAL
        ) {
        ...
        user.hives = user.hives.add(
            honeyForCompound.div(HONEY_TO_BUILD_A_HIVE)
        );
        user.honey = 0;
    ...
    function sellHoney() public {
        ...
        if (user.compounds < COMPOUNDS_FOR_NO_PENALTY) {
            ...
            honeyValue = honeyValue.sub(
                honeyValue.mul(PENALTY_TAX).div(PERCENT_DIVISOR)
            );
        } else {
    ...
    function buildHives(address ref) public payable {
        ...
        require(
            user.initialDeposit.add(amount) <= MAX_DEPOSIT,
            "Max deposit limit reached."
        );

        uint256 honeyBought = calculateHoneyBuy(
            amount,
        ...
        if (user.referrer == address(0)) {
            ...
            if (upLine != address(0)) {
                users[upLine].referralsCount = users[upLine].referralsCount.add(
                    1
                );
            }
        }
        ...
        if (user.referrer != address(0)) {
            address upLine = user.referrer;
            if (upLine != address(0)) {
                uint256 refRewards = amount.mul(REFERRAL_REWARD_PERCENT).div(
                    PERCENT_DIVISOR
                );
                payable(upLine).transfer(refRewards);
                users[upLine].referralReward = users[upLine].referralReward.add(
                    refRewards
                );
    ...
    function getDailyCompoundBonus(
        ...
        if (users[_adr].compounds == 0) {
            ...
            uint256 totalBonus = users[_adr].compounds.mul(
                COMPOUND_BONUS_PERCENT
            );
    ...
    function initialize(uint256 amount) public payable onlyOwner {
        if (!contractStarted) {
    ...
    function getAvailableEarnings(address _adr) public view returns (uint256) {
        uint256 userHoney = users[_adr].honey.add(
            getHoneySinceLastCompound(_adr)
        );
    ...
    function calculateHoneyBuySimple(
        uint256 eth
    ) public view returns (uint256) {
        return calculateHoneyBuy(eth, getBalance());
    }

    function getHoneyYield(
        uint256 amount
    ) public view returns (uint256, uint256) {
        uint256 honeyAmount = calculateHoneyBuy(
            amount,
        ...
        uint256 earningsPerDay = calculateHoneySellForYield(
            honeyPerDay,
            amount
        );
    ...
    function calculateHoneySellForYield(
        uint256 honey,
        uint256 amount
    ) public view returns (uint256) {
    ...
    function getMyHoney() public view returns (uint256) {
        return
            users[msg.sender].honey.add(getHoneySinceLastCompound(msg.sender));
    }

    function getHoneySinceLastCompound(
        address adr
    ) public view returns (uint256) {
        uint256 secondsSinceLastCompound = block.timestamp.sub(
            users[adr].lastCompoundTimestamp
        );
        /** get min time. **/
    ...
    function SET_HONEY_TO_BUILD_A_HIVE(uint256 value) external onlyOwner {
        require(value >= 479520 && value <= 2592000);
        /** min 3% max 12%**/
        HONEY_TO_BUILD_A_HIVE = value;
    }

    function SET_TAX(uint256 value) external onlyOwner {
        require(value <= 100);
        /** 10% max **/
        TAX = value;
    }
    ...
    function SET_MARKET_HONEY_DIVISOR(uint256 value) external onlyOwner {
        require(value <= 50);
        /** 50 = 2% **/
    ...
    /** withdrawal tax **/
    function SET_PENALTY_WITHDRAWAL_TAX(uint256 value) external onlyOwner {
        require(value <= 800);
        /** Max Tax is 80% or lower **/
        PENALTY_TAX = value;
    }

    function SET_COMPOUNDS_FOR_NO_TAX_WITHDRAWAL(
        uint256 value
    ) external onlyOwner {
        require(value <= 25);
        /** Max 25 compounds **/
    ...
    function SET_COMPOUND_BONUS_MAX_TIMES(uint256 value) external onlyOwner {
        require(value <= 30);
        /** Max 30 times **/
    ...
    function SET_MIN_DEPOSIT(uint256 value) external onlyOwner {
        MIN_DEPOSIT = value * 1 ether;
    }

    function SET_MAX_DEPOSIT(uint256 value) external onlyOwner {
        require(value <= 20);
        /** Max 20 ETH **/
        MAX_DEPOSIT = value * 1 ether;
    }

    function SET_CUTOFF_TIMEOUT(uint256 value) external onlyOwner {
