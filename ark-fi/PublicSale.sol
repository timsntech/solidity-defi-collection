...
contract ARK_PUBLIC_SALE is ERC721, ERC721Enumerable {
    ...
    modifier onlyOwner() {
        if (!hasAdminRights[msg.sender]) return;
        _;
    }
    ...
    function tokenURI(
        ...
        return
            string(
                abi.encodePacked(baseURI, tokenId.toString(), _fileExtension)
            );
    }
    ...
    function mint(uint256 amount, address referrer) external {
        ...
        require(
            nftsOfWalletOfRound[msg.sender][currentRound] + amount <=
                maxNftsOfRoundPerWallet[currentRound],
            "Max NFT per wallet exceeded"
        );
        require(
            nftsLeftOfRound[currentRound] >= amount,
            "This round is sold out, sorry."
        );

        BUSD.transferFrom(
            msg.sender,
            address(this),
            amount * nftPriceOfRound[currentRound]
        );
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            if (referrer != msg.sender && referrer != address(0))
                referrerOf[idOfMintedNFT] = referrer;
            emit NftMinted(
                msg.sender,
                idOfMintedNFT,
                referrerOf[idOfMintedNFT],
                currentRound,
                nftPriceOfRound[currentRound]
            );
        }
    }
    ...
    function increaseLimitOfNFTsPerWallet(
        uint256 round,
        uint256 howMany
    ) external onlyOwner {
    ...
    function sell(uint256 id, uint256 price) external {
        require(
            ...
            "Can't transfer a token that is not owned by you"
        );
        idForSale[id] = true;
        priceOfId[id] = price;
        addNftForSale(id);
    ...
    function removeNftForSale(uint256 _nftForSale) internal {
        nftsForSale[nftForSaleIndexes[_nftForSale]] = nftsForSale[
            nftsForSale.length - 1
        ];
        nftForSaleIndexes[
            nftsForSale[nftsForSale.length - 1]
        ] = nftForSaleIndexes[_nftForSale];
        nftsForSale.pop();
    }

    // emergency functions just in case
    function rescueAnyToken(address tokenToRescue) external onlyOwner {
        IBEP20(tokenToRescue).transfer(
            msg.sender,
            IBEP20(tokenToRescue).balanceOf(address(this))
        );
    }
