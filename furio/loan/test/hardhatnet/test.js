...
describe("Contract deployment and initial settings", () => {
  it("FURFI Token deploy", async () => {
    ...
    furFiToken = await upgrades.deployProxy(FURFIToken, [
      "FurioFinanceToken",
      "$FURFI",
      toBigNum("20000"),
      owner.address,
  ...
  it("CommunityIssuance deploy", async () => {
    ...
    communityIssuance = await upgrades.deployProxy(CommunityIssuance);
  ...
  it("LoanStaking deploy", async () => {
    ...
    loanStaking = await upgrades.deployProxy(LoanStaking);
    await loanStaking.deployed();
  ...
  it("LOAN Token deploy", async () => {
    ...
    loanToken = await upgrades.deployProxy(LoanToken, [
  ...
  it("BorrowerOperations deploy", async () => {
    ...
    borrowerOperations = await upgrades.deployProxy(BorrowerOperations);
  ...
  it("FURUSD token deploy", async () => {
    ...
    furUSDToken = await upgrades.deployProxy(FurUSDToken, [
      vaultManager.address,
      stabilityPool.address,
      borrowerOperations.address,
      toBigNum("200000")
    ]);
    await furUSDToken.deployed();
  ...
  it("CollSurplusPool deploy", async () => {
    ...
    collSurplusPool = await upgrades.deployProxy(CollSurplusPool);
  ...
  it("SortedVaults deploy", async () => {
    ...
    sortedVaults = await upgrades.deployProxy(SortedVaults);
  ...
  it("HintHelpers deploy", async () => {
    ...
    hintHelpers = await upgrades.deployProxy(HintHelpers);
  ...
  it("MultiVaultGetter deploy", async () => {
    ...
    multiVaultGetter = await MultiVaultGetter.deploy();
    await multiVaultGetter.deployed();
