...
contract StableFund is Ownable, ReentrancyGuard {
    ...
    function claimAllReward() public nonReentrant {
        ...
        for (uint256 i; i < ownedDeposits[msg.sender].length; i++) {
            uint256 claimableReward = getClaimableReward(
                ownedDeposits[msg.sender][i]
            );
            allClaimableReward += claimableReward;
            depositState[ownedDeposits[msg.sender][i]]
                .claimedAmount += claimableReward;
        }
    ...
    function getAllClaimableReward(
        ...
        for (uint256 i = 0; i < ownedDeposits[investor].length; i++) {
            allClaimableReward += getClaimableReward(
                ownedDeposits[investor][i]
            );
        }
    ...
    function getTotalInvests() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < _currentDepositID; i++) {
            if (depositState[i + 1].state)
                totalInvests += depositState[i + 1].depositAmount;
        }
        return totalInvests;
    }
